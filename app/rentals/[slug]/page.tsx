...
export default function BusinessPage() {
  ...
  const fetchBusinessData = async () => {
    try {
      const response = await fetch(`/api/businesses/${params.slug}?category=rentals`)
    ...
    return (
      <div className="min-h-screen bg-gray-50 py-8 px-4">
        <div className="max-w-6xl mx-auto">
          ...
          <Link href="/rentals" className="text-blue-600 hover:text-blue-800">
            ← Back to Car Rentals
          </Link>
        </div>
      </div>
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-8 px-4">
      <div className="max-w-6xl mx-auto">
        <Link href="/rentals" className="text-blue-600 hover:text-blue-800 mb-4 inline-block">
          ← Back to Car Rentals
        </Link>
          ...
          {reviews.map((review) => (
            <div key={review.id} className="bg-white rounded-lg shadow-md p-6">
              ...
              <div className="flex justify-between items-center text-sm text-gray-500">
                ...
                <Link
                  href={`/rentals/${params.slug}/review/${review.id}`}
